...
新增工具链：
├── nu2cpp:   Nu代码 → C++代码 (现代C++代码生成) ⭐ 核心
├── nu2cmake: nu.toml → CMakeLists.txt (构建配置转换) ⭐ 核心
├── cpp2nu:   C++代码 → Nu代码 (可选，Phase 2)
└── cmake2nu: CMakeLists.txt → nu.toml (可选，反向转换)
...
pub mod cmake;       // CMake生成器 ⭐
pub mod workspace;   // Workspace支持 ⭐

// src/bin/nu2cmake.rs ⭐ 新增
// nu.toml → CMakeLists.txt 转换工具
```
...
### 7.12 nu.toml → CMakeLists.txt 转换

转换器支持将 Nu 项目配置文件 `nu.toml` 自动转换为 CMake 构建配置。
...
```cmake
cmake_minimum_required(VERSION 3.15)
project(my_app VERSION 0.1.0 LANGUAGES CXX)
...
**生成的 CMakeLists.txt (workspace 根)**:
```cmake
cmake_minimum_required(VERSION 3.15)
project(MyWorkspace VERSION 1.0.0 LANGUAGES CXX)
...
add_library(lib1 ${SOURCES} ${HEADERS})
...
**生成的 lib2/CMakeLists.txt**:
```cmake
project(lib2 VERSION 0.1.0)

file(GLOB_RECURSE SOURCES "src/*.cpp")
file(GLOB_RECURSE HEADERS "src/*.hpp")

add_library(lib2 ${SOURCES} ${HEADERS})
...
| Nu.toml | CMakeLists.txt | 说明 |
|---------|---------------|------|
| `[P]` | `project()` | 项目定义 |
| `n =` | 项目名称 | Project name |
| `v =` | `VERSION` | 版本号 |
| `e =` | `CMAKE_CXX_STANDARD` | C++ 标准（2015→14, 2018→17, 2021→20） |
| `[D]` | `find_package()` + `target_link_libraries()` | 依赖库 |
| `[BD]` | 构建依赖（如 CMake 版本要求） | Build dependencies |
| `[W]` | workspace 根标记 | Workspace marker |
| `m =` | `add_subdirectory()` | 成员列表 |
| `[workspace.dependencies]` | 根级 `find_package()` | 共享依赖 |
...
# → 使用根 CMakeLists.txt 的 find_package

# 路径依赖
my_lib = { path = "../my_lib" }
# → target_link_libraries(... my_lib)
# → add_subdirectory(../my_lib) (在根级)

# 可选依赖
opencv = { version = "4.0", optional = true }
# → find_package(OpenCV 4.0)
# → if(OpenCV_FOUND) target_link_libraries(...) endif()
```
...
[D]
tbb = { version = "2021", optional = true }
...
option(ENABLE_PARALLEL "Enable parallel processing" OFF)

if(ENABLE_PARALLEL)
    find_package(TBB 2021 REQUIRED)
    target_link_libraries(my_app PRIVATE TBB::tbb)
    target_compile_definitions(my_app PRIVATE ENABLE_PARALLEL)
endif()
...
```bash
# nu2cmake - 转换 nu.toml 到 CMakeLists.txt
nu2cmake path/to/nu_project

# 输出
✓ 检测到 Workspace 结构
✓ 找到 3 个成员: lib1, lib2, app
✓ 生成 CMakeLists.txt (根)
✓ 生成 lib1/CMakeLists.txt
✓ 生成 lib2/CMakeLists.txt
✓ 生成 app/CMakeLists.txt

# 验证构建
cd path/to/nu_project
cmake -B build
cmake --build build
```
...
```bash
# 一键转换整个 Nu 项目到 C++
nu2cpp --with-cmake my_nu_project/ output_cpp/

# 等价于
nu2cpp my_nu_project/ output_cpp/  # 转换代码
nu2cmake output_cpp/                # 生成 CMake 配置
...
### Phase 0: 最小闭环验证 (1-2周) ⭐ 最高优先级

**目标**: 打通 "Nu → C++ → CMake → 可执行文件" 完整流程

**理由**: 先建立信心！一个能跑通的 Hello World 比复杂的类型系统更重要。一旦打通构建流，后续添加语法特性只是填空题。

- [ ] Week 1: 最小转换器
  - [ ] **复用现有 Lexer/Parser**（来自 rust2nu/nu2rust）⭐
  - [ ] 最简 AST 遍历（只处理函数和 println!）
  - [ ] 最简 C++ 代码生成器
    ```nu
    f main() {
      println!("Hello, Nu2CPP!")
    }
    ```
    →
    ```cpp
    #include <iostream>
    int main() {
      std::cout << "Hello, Nu2CPP!" << std::endl;
      return 0;
    }
    ```
  
- [ ] Week 2: CMake 集成
  - [ ] **nu2cmake 工具原型**（nu.toml → CMakeLists.txt）⭐
  - [ ] 最简 CMakeLists.txt 生成
    ```cmake
    cmake_minimum_required(VERSION 3.15)
    project(HelloNu CXX)
    set(CMAKE_CXX_STANDARD 17)
  ...
  - [ ] 端到端测试：
    ```bash
    nu2cpp hello.nu output/
    cd output && cmake -B build && cmake --build build
    ./build/hello  # 输出: Hello, Nu2CPP!
    ```

**里程碑**: 🎉 第一个 Nu 程序成功编译并运行！
...
### Phase 1: 基础类型与表达式 (3-4周)

**目标**: 扩展到基本类型和简单表达式

- [ ] Week 3-4: 核心类型支持
  - [ ] 基本类型映射（i32, u32, f64, bool, String）
  - [ ] 变量声明（l/v）
  - [ ] 简单表达式（算术、比较、逻辑）
  - [ ] if/else 控制流
...
### Phase 2: 第三方库与构建集成 (3-4周)

**目标**: 支持 @import 和完整 CMake 生成

- [ ] Week 7-8: @import 机制
  - [ ] @import 语法解析
  - [ ] 内置库映射表（fmt, vector, string 等）
  - [ ] #include 自动生成
  - [ ] nu_libs.toml 配置支持
  
- [ ] Week 9-10: CMake 完善与 Workspace
  - [ ] nu.toml 字段完整映射
  - [ ] Workspace 结构支持（[W], m=）
  - [ ] 依赖关系处理（path, workspace）
  - [ ] 完整的 CMakeLists.txt 生成

**里程碑**: Todo List 示例成功转换并编译运行（使用 fmt 库）

### Phase 3: 高级语法与集合 (5-6周)
...
**里程碑**: 转换中等复杂度项目（如命令行工具）

### Phase 4: 运行时库与工具完善 (3-4周)
...
- [ ] Week 17-18: 运行时库
  ...
  - [ ] Option/Result 辅助函数
  - [ ] 迭代器适配器
  - [ ] println!/panic!/assert! 宏
  
- [ ] Week 19-20: 测试、文档与发布
  - [ ] 单项目集成测试
  - [ ] Workspace 集成测试
  - [ ] 用户文档（完整教程）
  - [ ] 示例项目集（Hello World → Todo List → CLI Tool）
  - [ ] 性能基准测试
...
- **Phase 0**: 1-2 周（最小闭环）⭐
- **Phase 1**: 3-4 周（基础类型）
- **Phase 2**: 3-4 周（@import + CMake）
- **Phase 3**: 5-6 周（高级语法）
- **Phase 4**: 3-4 周（运行时库）

**总计**: 15-20 周（比原计划缩短 2-4 周）

**关键改进**:
1. ✅ Phase 0 快速验证可行性（1-2 周即可看到成果）
2. ✅ 优先打通构建流程（CMake 提前到 Phase 2）
3. ✅ 延后复杂特性（泛型、trait 等）
4. ✅ 更快的迭代反馈周期
...
### 🔴 Phase 0 优先级（最小闭环 - 1-2周）

**信心建立优先**：
1. ✅ **Lexer/Parser 复用**（来自 rust2nu）⭐ 最关键
2. ✅ **最简函数转换**（f main + println!）
3. ✅ **最简 C++ 生成**（iostream + main）
4. ✅ **nu2cmake 原型**（nu.toml → CMakeLists.txt）⭐ 最关键
5. ✅ **端到端测试**（Hello World 编译运行）

**目标**: 1-2 周内看到第一个成功运行的 Nu 程序！

---

### 🟠 Phase 1 优先级（基础类型 - 3-4周）

6. ✅ 基本类型映射（i32, String, bool）
7. ✅ 变量声明（l/v）
8. ✅ 简单表达式
9. ✅ if/else 控制流
10. ✅ 函数定义与调用
...
### 🟡 Phase 2 优先级（第三方库 - 3-4周）

12. ✅ **@import 语法解析** ⭐
13. ✅ **内置库映射表**（fmt, vector, string）⭐
14. ✅ **#include 自动生成** ⭐
15. ✅ **CMakeLists.txt 完善** ⭐
16. ✅ **Workspace 支持**（[W], m=）⭐
17. ✅ nu_libs.toml 配置

---

### 🟢 Phase 3 优先级（高级语法 - 5-6周）

18. match 模式匹配
19. Vec/HashMap/HashSet
20. Option/Result
21. 智能指针（Box/Arc）
22. impl 块转换
23. 基础泛型
...
### 🔵 Phase 4 优先级（运行时库 - 3-4周）

24. nu_runtime 库
25. 迭代器适配器
26. println!/panic!/assert!
27. 完整测试套件
28. 用户文档
...
29. cpp2nu 反向转换
30. 高级泛型与 trait → concept
31. 条件导入（@import[cfg]）
32. 导入别名（as 语法）
33. C++20 modules 支持
34. 包管理器集成（vcpkg/Conan）
35. 编译优化
36. 调试器集成
...
**核心变化**：
- 🔴 **提前 CMake 集成**：从 Phase 4 提到 Phase 0，快速验证可行性
- 🔴 **复用现有代码**：Lexer/Parser 直接复用，节省 4-6 周
- 🟡 **@import 提前**：从 Phase 1 独立成 Phase 2，重点投入
- 🟢 **延后复杂特性**：泛型、trait 等推迟到 Phase 3+
...
- ✨ 完整的 @import 第三方库导入机制
- ✨ 自动化 CMake 配置生成（支持 Workspace）
- ✨ 零成本抽象，保持 C++ 性能

**开发策略**（🎯 重点优化）：
- 🔴 **Phase 0 (1-2周)**: 最小闭环验证（Hello World + CMake）⭐
- 🟠 **Phase 1 (3-4周)**: 基础类型与表达式
- 🟡 **Phase 2 (3-4周)**: @import + Workspace 支持 ⭐
- 🟢 **Phase 3 (5-6周)**: 高级语法与集合
- 🔵 **Phase 4 (3-4周)**: 运行时库完善
...
- 新计划：**15-20 周** ⭐
- 节省：2-4 周
- **关键改进**: 1-2 周即可看到第一个成功运行的程序！

预计总开发时间：**15-20 周**（优化后）
核心团队规模：**2-3 人**
技术栈：**Rust (转换器) + C++17/20 (运行时)**
复用资源：**Lexer/Parser 完全复用** ⭐
