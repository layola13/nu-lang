...
fn test_simple_workspace_cargo2nu() {
    ...
    assert!(
        verify_structure(Path::new(output), &expected),
...
fn test_simple_workspace_roundtrip() {
    ...
    // Nu -> Cargo
    assert!(
        run_nu2cargo(nu_output, cargo_output, &[]),
        "nu2cargo failed"
    );
    ...
    assert!(
        verify_structure(Path::new(cargo_output), &expected),
        "Roundtrip structure verification failed"
    );
...
fn test_simple_workspace_dry_run() {
    ...
    // 运行 dry-run
    assert!(
        run_cargo2nu(input, output, &["--dry-run"]),
        "cargo2nu dry-run failed"
    );

    // 验证没有创建任何文件
    assert!(
        !Path::new(output).exists() || fs::read_dir(output).unwrap().count() == 0,
...
fn test_simple_workspace_incremental() {
    ...
    // 增量转换（应该跳过）
    assert!(
        run_cargo2nu(input, output, &["-i"]),
        "Incremental conversion failed"
    );
    ...
    assert_eq!(
        mtime1, mtime2,
        "File should not be modified in incremental mode"
    );
}
...
fn test_simple_workspace_exclude() {
    ...
    // 排除 lib2
    assert!(
        run_cargo2nu(input, output, &["--exclude", "lib2"]),
        "Exclude conversion failed"
    );

    // 验证 lib1 存在，lib2 不存在
    assert!(
        Path::new(&format!("{}/lib1", output)).exists(),
        "lib1 should exist"
    );
    assert!(
        !Path::new(&format!("{}/lib2", output)).exists(),
        "lib2 should not exist"
    );
}
...
fn test_simple_workspace_only() {
    ...
    // 只转换 lib1
    assert!(
        run_cargo2nu(input, output, &["--only", "lib1"]),
        "Only conversion failed"
    );
...
fn test_toml_section_conversion() {
    ...
    assert!(
        !nu_toml.contains("members = "),
        "Should not have 'members' key"
    );
}
