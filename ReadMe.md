...
| **!** | Mut/Try | `mut`/`?` | **前缀**=mut (`!self`, `&!x`), **后缀**=try (`func()!`) |
...
| 符号 | 含义 | 规则 | 示例 |
| --- | --- | --- | --- |
| **!** | **Mut** (前缀) | 变量/引用/指针的可变性 | `&!self` (&mut self), `*!ptr` (*mut ptr), `!self` (mut self) |
| **!** | **Try** (后缀) | 错误传播 | `func()!` (func()?) |
| **U** | **Unsafe** | 不安全块 | `U { ... }` |
| **&** | Ref | 引用 | `&x` |
| ***** | Deref | 解引用 | `*ptr` |
...
**注意**：`!` 符号的语义由位置决定：
- **前缀 `!`**：表示 `mut` (可变性修饰)
  - `&!self` → `&mut self` (可变引用self)
  - `!self` → `mut self` (按值接收的可变self)
  - `*!ptr` → `*mut ptr` (可变原始指针)
  - `!x` → `mut x` (可变变量，但推荐用 `v`)
- **后缀 `!`**：表示 `?` (错误传播)
  - `read_file()!` → `read_file()?`
