...
pub static SECTION_CARGO_TO_NU: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
...
pub static SECTION_NU_TO_CARGO: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
    let mut m = HashMap::new();
    // 反向映射
...
mod tests {
    ...
    fn test_section_cargo_to_nu() {
        ...
        assert_eq!(convert_section_cargo_to_nu("[workspace.dependencies]"), "[W.D]");
    ...
    fn test_section_nu_to_cargo() {
        ...
        assert_eq!(convert_section_nu_to_cargo("[W.D]"), "[workspace.dependencies]");
    ...
    fn test_section_roundtrip() {
        let sections = vec![
            ...
            "[build-dependencies]", "[workspace.dependencies]", "[workspace.package]",
            "[lib]", "[features]", "[[bin]]", "[[example]]", "[[test]]", "[[bench]]",
        ];
    ...
    fn test_key_roundtrip() {
        let keys = vec![
            "name", "version", "edition", "members", "exclude", "resolver",
            "workspace", "default-features", "optional", "authors", "description",
            "license", "repository", "documentation", "homepage", "keywords",
            "categories", "proc-macro", "crate-type", "required-features",
        ];
    ...
    fn test_preserved_sections() {
        // 保留节应该不变
        assert_eq!(convert_section_cargo_to_nu("[profile.release]"), "[profile.release]");
        assert_eq!(convert_section_cargo_to_nu("[patch.crates-io]"), "[patch.crates-io]");
        assert_eq!(convert_section_cargo_to_nu("[target.'cfg(windows)'.dependencies]"), 
